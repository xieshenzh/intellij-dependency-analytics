...
import static org.jboss.tools.intellij.componentanalysis.CAService.NO_VUL_DEP;
...
public abstract class CAAnnotator extends ExternalAnnotator<CAAnnotator.Info, Map<Dependency, CAAnnotator.Result>> {
    ...
    public @Nullable Map<Dependency, Result> doAnnotate(Info info) {
        if (info != null && info.getFile() != null
            ...
            Collection<Dependency> dependencies = info.getDependencies().keySet();
            Map<Dependency, DependencyReport> reports = CAService.getReports(path, dependencies);

            if (!reports.keySet().containsAll(dependencies)) {
    ...
    private Map<Dependency, Result> matchDependencies(Map<Dependency, List<PsiElement>> dependencies,
        ...
        if (dependencies != null && !dependencies.isEmpty()
            ...
            return dependencies.entrySet()
                    .parallelStream()
                    .filter(e -> {
                        DependencyReport report = reports.get(e.getKey());
                        return report != null && !NO_VUL_DEP.equals(report);
                    })
